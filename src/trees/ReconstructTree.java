...
public class ReconstructTree {
	...
	public static DefaultMutableTreeNode constructTree(String inorder,
			String preorder) {
		...
		if (rightSubtreeInorder.length() > 0
				&& rightSubtreePreorder.length() > 0)
			root.add(constructTree(rightSubtreeInorder, rightSubtreePreorder));
		return root;

	}

	public static String[] makePreorder(String preorder,
			String leftSubtreeInorder) {
		for (int i = 1; i < preorder.length(); i++) {
			if (leftSubtreeInorder.indexOf(preorder.charAt(i)) == -1) {
				return new String[] { preorder.substring(1, i),
						preorder.substring(i) };
			}
		}
